...
function solveOptimizationProblem_3(InputParameters::InputParam, SolverParameters::SolverParam, Battery::BatteryParam)
    ...
    @timeit to "Collecting results" begin
      ...
      #=  w_uu[end] = JuMP.value(problem.w_uu[end])
        ...
        cost_rev[NStages]= Battery_price_purchase[NStages]*rev[NStages]-Battery_price_sale[NStages]*(cap[end]-min_SOH/min_SOH)
...
function solveOptimizationProblem_4(InputParameters::InputParam, SolverParameters::SolverParam, Battery::BatteryParam)
    ...
    @timeit to "Collecting results" begin
        ...
        for iStage=2:(NStages-1)
            gain_stage[iStage] = sum(Power_prices[iStep]*NHoursStep*(discharge[iStep]*Eff_discharge-charge[iStep]/Eff_charge) for iStep=(Steps_stages[iStage]+1):(Steps_stages[iStage+1]))
            cost_rev[iStage] = Battery_price_purchase[iStage]*(cap[Steps_stages[iStage]+2]+rev_acquisto[iStage]) - Battery_price_sale[iStage]*(cap[Steps_stages[iStage]+1]-rev_vendita[iStage]) + e[iStage]*fix
        end

        gain_stage[1] = sum(Power_prices[iStep]*NHoursStep*(discharge[iStep]*Eff_discharge-charge[iStep]/Eff_charge) for iStep=(Steps_stages[1]+1):(Steps_stages[2]))
        cost_rev[1] = Battery_price_purchase[1]*rev[1] + fix*e[1]

        gain_stage[NStages]= sum(Power_prices[iStep]*NHoursStep*(discharge[iStep]*Eff_discharge-charge[iStep]/Eff_charge) for iStep=(Steps_stages[NStages]+1):(Steps_stages[NStages+1]))
